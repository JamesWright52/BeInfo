...
int inter_OM(POINT m_courant, POINT point_bord_1, POINT point_bord_2){
  ...
  if ( ( (point_bord_1.x <= 0) && (point_bord_2.x <= 0) )
    || ( (point_bord_1.y <= 0) && (point_bord_2.y <= 0) ){
      return 0;
    }
    ...
    if ( (m_courant.x <= 0) || (m_courant.y <= 0) ){
      return 0;
    }

  // pente de la droite (OM)
  double pente_M = m_courant.y/m_courant.x;
  double pente_P1 = point_bord_1.y/point_bord_1.x;
  double pente_P2 = point_bord_2.y/point_bord_2.x;
  ...
  if (point_bord_1.x == point_bord_2.x){
    ...
    if ( minim(point_bord_1,point_bord_2) < m_courant.x){
...
int nombredinter(LISTE* perimetre, POINT m_courant){
  ...
  while(NULL != element_parcouru){
    if ( inter_OM(m_courant, element_parcouru->p, element_parcouru->suivant->p) == 1 ){
      compt += 1;
    }
    ...
    if ( (element_parcouru != NULL) && (element_parcouru->suivant == NULL) ){
      if ( inter_OM(m_courant, element_parcouru->p, perimetre->premier.p) == 1 ){
        compt += 1;
      }
    }
  return compt;
...
LISTE* liste_interieur(LISTE* perimetre, double x_plateau, double y_plateau){
  ...
  for (i=0; i < x_plateau; i++){
    for (j=0; j < y_plateau ; j++){
      ...
      if (dedans(perimetre,tab_plateau[i][j]) == 1) {
