        ...
        if ( (z1 == cote) && (z2 == cote) && (z3 == cote) ){
         ...
         //sinon on calcul l'intersection du plan avecle triangle
            ...
            if (segment_intersecte(cote,z1,z2) == 1){
               ...
               push_head(intersection, (( (cote - z1) * matrice[i].p2.x - (cote - z2) * matrice[i].p1.x ) / (z2-z1) ));
               push_head(intersection, (( (cote - z1) * matrice[i].p2.y - (cote - z2) * matrice[i].p1.y ) / (z2-z1) ));
               push_head(intersection, cote);
               }
            if (segment_intersecte(cote,z2,z3) == 1) {
               push_head(intersection,(( (cote - z2) * matrice[i].p3.x - (cote - z3) * matrice[i].p2.x ) / (z3-z2) ));
               push_head(intersection,(( (cote - z2) * matrice[i].p3.y - (cote - z3) * matrice[i].p2.y ) / (z3-z2) ));
               push_head(intersection,cote);
               }
            if (segment_intersecte(cote,z3,z1) == 1) {
               push_head((intersection, (cote - z3) * matrice[i].p1.x - (cote - z1) * matrice[i].p3.x ) / (z1-z3) );
               push_head((intersection, (cote - z3) * matrice[i].p1.y - (cote - z1) * matrice[i].p3.y ) / (z1-z3) );
               push_head(intersection, cote);
               }
         }
     }
